 ...
 */
angular.module('gov.va.iam.acs.ams.filters.messages', []).filter('uniqueMessage', function(){
    'use strict';
    ...
    return function (messages) {
        ...
        if(Object.isArray(messages) && messages.length > 0){
            filteredArray = messages.filter(function(message, messageIndex, messageArray) {
                if(message.getType().toLowerCase() === VA_AMS.models.Message.ERROR) {
                    if(!Object.isDefined(message.getValue())) {
                        message.setValue(VA_AMS.models.Message.ERROR_MSG);
                    }
                    filterResult = true;
                } else {
                    filterResult = false;
